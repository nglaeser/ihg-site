...
The `scripts/` directory is not related to Jekyll but was added by me. It contains a pre-push hook related to simplifying deployment of the website (see below).
...
To view local edits to the site, run
```
...
To simplify this process, I've made a pre-push [Git hook](https://githooks.com/) that automatically zips the latest version of `_site/` to create a `.zip` file that can be easily uploaded to the web server and then unzipped there. The script for this is [`scripts/pre-push.sh`](scripts/pre-push.sh). If you clone this repo and wish to use the Git hook as well, "install" the hook by running
```
ln -s ../../scripts/pre-push.sh .git/hooks/pre-push
```
in the root of the repo. This will make sure the script is automatically run before every `git push`.